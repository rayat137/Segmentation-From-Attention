...
class Compose(object):
    ...
    def __call__(self, image, label=None, original_label=None):
        if label is None:
            ...
            for t in self.segtransform:
                ...
                if original_label is not None:
                    image, label, original_label = t(image, label, original_label)
                else:
...
class ToTensor(object):
    ...
    def __call__(self, image, label, original_label=None):
        ...
        if label is not None:
            ...
            if original_label is not None:
                if not isinstance(original_label, np.ndarray):
                    raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray"
                                    "[eg: data readed by cv2.imread()].\n"))
                if not len(original_label.shape) == 2:
                    raise (RuntimeError("segtransform.ToTensor() only handle np.ndarray labellabel with 2 dims.\n"))
                original_label = torch.from_numpy(original_label)
                if not isinstance(original_label, torch.LongTensor):
                    original_label = original_label.long()
                return image, label, original_label
...
class Resize(object):
    ...
    def __call__(self, image, label, original_label = None):
        ...
        if label is not None:
            ...
            if original_label is not None:
                s_mask = original_label
                new_h, new_w = find_new_hw(s_mask.shape[0], s_mask.shape[1], test_size)
                s_mask = cv2.resize(s_mask.astype(np.float32), dsize=(int(new_w), int(new_h)),
                                    interpolation=cv2.INTER_NEAREST)
                back_crop_s_mask = np.ones((test_size, test_size)) * 255
                back_crop_s_mask[:new_h, :new_w] = s_mask
                original_label = back_crop_s_mask
                return image, label, original_label
...
class Crop(object):
    ...
    def __call__(self, image, label, original_label=None):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            if original_label is not None:
                original_label = cv2.copyMakeBorder(original_label, pad_h_half, pad_h - pad_h_half, pad_w_half,
                                        pad_w - pad_w_half, cv2.BORDER_CONSTANT, value=self.ignore_label)
        h, w = image.shape[:2]
        ...
        if label is not None:
            ...
            if original_label is not None:
                original_label = original_label[h_off:h_off+self.crop_h, w_off:w_off+self.crop_w]
                return image, label, original_label
